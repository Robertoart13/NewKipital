...
export class CreatePayrollAndPersonalActions1708531800000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.createIndex(
      'nom_planillas',
      new TableIndex({
        name: 'IDX_planilla_periodo_pago',
    ...
    await queryRunner.createIndex(
      'nom_planillas',
      new TableIndex({
        name: 'IDX_planilla_estado',
        columnNames: ['estado_planilla'],
      }),
    );
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          {
            name: 'fecha_aprobacion_accion',
            type: 'datetime',
            isNullable: true,
          },
          { name: 'motivo_rechazo_accion', type: 'text', isNullable: true },
          {
            name: 'fecha_creacion_accion',
            type: 'datetime',
            default: 'CURRENT_TIMESTAMP',
          },
          {
            name: 'fecha_modificacion_accion',
            type: 'datetime',
            default: 'CURRENT_TIMESTAMP',
            onUpdate: 'CURRENT_TIMESTAMP',
          },
    ...
    await queryRunner.createIndex(
      'acc_acciones_personal',
      new TableIndex({
        name: 'IDX_accion_estado',
        columnNames: ['estado_accion'],
      }),
    );
