...
export class CreateCalendarioNominaMaestro1708531900000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.createTable(new Table({
      ...
      columns: [
        { name: 'id_calendario_nomina', type: 'int', isPrimary: true, isGenerated: true, generationStrategy: 'increment' },
        { name: 'id_empresa', type: 'int', isNullable: false },
        { name: 'id_periodos_pago', type: 'int', isNullable: false },
        { name: 'tipo_planilla', type: 'varchar', length: '30', isNullable: false, default: "'Regular'" },
        ...
        { name: 'moneda_calendario_nomina', type: 'enum', enum: ['CRC', 'USD'], default: "'CRC'" },
        { name: 'estado_calendario_nomina', type: 'tinyint', width: 1, default: 1 },
        { name: 'es_inactivo', type: 'tinyint', width: 1, default: 0 },
        { name: 'descripcion_evento_calendario_nomina', type: 'text', isNullable: true },
        { name: 'etiqueta_color_calendario_nomina', type: 'varchar', length: '20', isNullable: true },
        { name: 'prioridad_calendario_nomina', type: 'int', isNullable: true },
        { name: 'fecha_aplicacion_calendario_nomina', type: 'datetime', isNullable: true },
        { name: 'fecha_creacion_calendario_nomina', type: 'datetime', default: 'CURRENT_TIMESTAMP' },
        { name: 'fecha_modificacion_calendario_nomina', type: 'datetime', default: 'CURRENT_TIMESTAMP', onUpdate: 'CURRENT_TIMESTAMP' },
        { name: 'creado_por_calendario_nomina', type: 'int', isNullable: true },
        { name: 'modificado_por_calendario_nomina', type: 'int', isNullable: true },
      ],
    }), true);

    await queryRunner.createIndex('nom_calendarios_nomina', new TableIndex({ name: 'IDX_calendario_empresa', columnNames: ['id_empresa'] }));
    await queryRunner.createIndex('nom_calendarios_nomina', new TableIndex({ name: 'IDX_calendario_periodo_pago', columnNames: ['id_periodos_pago'] }));
    await queryRunner.createIndex('nom_calendarios_nomina', new TableIndex({ name: 'IDX_calendario_estado', columnNames: ['estado_calendario_nomina'] }));
    await queryRunner.createIndex('nom_calendarios_nomina', new TableIndex({ name: 'IDX_calendario_periodo_inicio_fin', columnNames: ['fecha_inicio_periodo', 'fecha_fin_periodo'] }));

    await queryRunner.createForeignKey('nom_calendarios_nomina', new TableForeignKey({
    ...
    await queryRunner.createIndex('nom_calendarios_nomina', new TableIndex({
      name: 'UQ_calendario_slot_operativo',
      columnNames: ['id_empresa', 'id_periodos_pago', 'fecha_inicio_periodo', 'fecha_fin_periodo', 'moneda_calendario_nomina', 'tipo_planilla'],
    ...
    await queryRunner.query('ALTER TABLE acc_acciones_personal CHANGE COLUMN id_planilla id_calendario_nomina INT NULL');
    ...
    await queryRunner.createTable(new Table({
      ...
      columns: [
        { name: 'id_cuota', type: 'int', isPrimary: true, isGenerated: true, generationStrategy: 'increment' },
        ...
        { name: 'monto_cuota', type: 'decimal', precision: 12, scale: 2, isNullable: false },
        ...
        { name: 'fecha_creacion_cuota', type: 'datetime', default: 'CURRENT_TIMESTAMP' },
        { name: 'fecha_modificacion_cuota', type: 'datetime', default: 'CURRENT_TIMESTAMP', onUpdate: 'CURRENT_TIMESTAMP' },
      ],
    }), true);

    await queryRunner.createIndex('acc_cuotas_accion', new TableIndex({ name: 'IDX_cuota_accion', columnNames: ['id_accion'] }));
    await queryRunner.createIndex('acc_cuotas_accion', new TableIndex({ name: 'IDX_cuota_calendario', columnNames: ['id_calendario_nomina'] }));
    await queryRunner.createIndex('acc_cuotas_accion', new TableIndex({ name: 'IDX_cuota_estado', columnNames: ['estado_cuota'] }));

    await queryRunner.createForeignKey('acc_cuotas_accion', new TableForeignKey({
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query('ALTER TABLE acc_acciones_personal CHANGE COLUMN id_calendario_nomina id_planilla INT NULL');
