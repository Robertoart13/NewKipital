  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const rolesKpital = [
      [
        'GERENTE_NOMINA',
        'Gerente de Nómina',
        'Gestión estratégica de nómina, supervisión y validación. Sin control total de configuración técnica.',
      ],
      [
        'OPERADOR_NOMINA',
        'Operador de Nómina',
        'Tareas operativas: aplicar nóminas, revisar datos, gestión de registros a nivel operativo.',
      ],
    ];
    ...
    const rolesTimewise = [
      [
        ...
        'Dueño o cabeza de la empresa. Ver y aprobar todo en su alcance.',
      ],
      [
        'SUPERVISOR_TIMEWISE',
        'Supervisor TimeWise',
        'Empleados con permisos para gestionar a sus empleados (jefes de área).',
      ],
      [
        'EMPLEADO_TIMEWISE',
        'Empleado TimeWise',
    ...
    const masterRows = await queryRunner.query(
    ...
    const gerenteRows = await queryRunner.query(
    ...
    if (gerenteRow) {
      ...
      for (const cod of gerentePerms) {
        const pRows = await queryRunner.query(
          `SELECT id_permiso FROM sys_permisos WHERE codigo_permiso = '${cod}' LIMIT 1`,
        );
        if (pRows.length > 0) {
          const p = pRows[0];
          const ex = await queryRunner.query(
            `SELECT 1 FROM sys_rol_permiso WHERE id_rol = ${gerenteRow.id_rol} AND id_permiso = ${p.id_permiso}`,
          );
          if (ex.length === 0) {
            await queryRunner.query(
    ...
    const operadorRows = await queryRunner.query(
    ...
    if (operadorRow) {
      ...
      for (const cod of operadorPerms) {
        ...
        if (pRows.length > 0) {
          const p = pRows[0];
          const ex = await queryRunner.query(
            `SELECT 1 FROM sys_rol_permiso WHERE id_rol = ${operadorRow.id_rol} AND id_permiso = ${p.id_permiso}`,
          );
          if (ex.length === 0) {
            await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const cod of rolesToRemove) {
      const rows = await queryRunner.query(
        `SELECT id_rol FROM sys_roles WHERE codigo_rol = '${cod}'`,
      );
      for (const r of rows) {
