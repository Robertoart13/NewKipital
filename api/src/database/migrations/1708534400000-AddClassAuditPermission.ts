...
export class AddClassAuditPermission1708534400000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      ...
      VALUES
        ('config:clases:audit', 'Ver bitacora de clases', 'Permite ver la bitacora de cambios de clases organizacionales', 'config', 1, '${now}')
    ...
    await queryRunner.query(`
      ...
      FROM sys_roles r
      INNER JOIN sys_permisos p ON p.codigo_permiso = 'config:clases:audit'
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      ...
      WHERE p.codigo_permiso = 'config:clases:audit'
    `);

    await queryRunner.query(`
      DELETE FROM sys_permisos
      WHERE codigo_permiso = 'config:clases:audit'
    `);
  }
}
