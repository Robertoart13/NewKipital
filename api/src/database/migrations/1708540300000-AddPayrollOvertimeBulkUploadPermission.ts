...
/**
 * Permiso dedicado para acceso a la vista de carga masiva de horas extras.
 */
export class AddPayrollOvertimeBulkUploadPermission1708540300000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      INSERT INTO sys_permisos (
        ...
        'Permite acceder a la vista de carga masiva de horas extras',
        'payroll',
        1,
        '${now}'
      )
    ...
    await queryRunner.query(`
      ...
      FROM sys_roles r
      INNER JOIN sys_permisos p ON p.codigo_permiso = 'payroll:overtime:bulk-upload'
      WHERE r.codigo_rol IN ('MASTER', 'ADMIN_SISTEMA', 'GERENTE_NOMINA', 'OPERADOR_NOMINA')
        AND r.estado_rol = 1
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      ...
      WHERE p.codigo_permiso = 'payroll:overtime:bulk-upload'
    `);

    await queryRunner.query(`
      DELETE FROM sys_permisos
      WHERE codigo_permiso = 'payroll:overtime:bulk-upload'
    `);
  }
}
