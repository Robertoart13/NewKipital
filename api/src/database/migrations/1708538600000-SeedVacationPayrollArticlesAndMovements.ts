...
export class SeedVacationPayrollArticlesAndMovements1708538600000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      ...
      SELECT
        e.id_empresa,
        'Articulo de nomina Vacaciones',
        'Articulo operativo para acciones de personal de vacaciones.',
        13,
        1,
      ...
      WHERE NOT EXISTS (
        ...
        WHERE a.id_empresa = e.id_empresa
          AND a.id_tipo_accion_personal = 13
          AND LOWER(a.nombre_articulo_nomina) = LOWER('Articulo de nomina Vacaciones')
      )
    `);
    ...
    await queryRunner.query(`
      ...
      SELECT
        ...
        NULL,
        NULL,
        'Movimiento de vacaciones por dias solicitados.',
        1,
        '1',
        '0',
        'Dias de vacaciones solicitados.',
        0
      FROM nom_articulos_nomina a
      WHERE a.id_tipo_accion_personal = 13
        AND a.es_inactivo = 0
        AND NOT EXISTS (
          SELECT 1
          FROM nom_movimientos_nomina m
          WHERE m.id_empresa_movimiento_nomina = a.id_empresa
            AND m.id_tipo_accion_personal_movimiento_nomina = 13
            AND LOWER(m.nombre_movimiento_nomina) = LOWER('Vacaciones')
        )
    `);
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      ...
      WHERE id_tipo_accion_personal_movimiento_nomina = 13
    ...
    await queryRunner.query(`
      ...
      WHERE id_tipo_accion_personal = 13
        AND LOWER(nombre_articulo_nomina) = LOWER('Articulo de nomina Vacaciones')
    `);
  }
}
