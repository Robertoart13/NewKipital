  ...
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      ...
      SELECT
        e.id_empresa,
        'Articulo de nomina Bonificacion',
        'Articulo operativo para acciones de personal de bonificaciones.',
        9,
        1,
      ...
      WHERE NOT EXISTS (
        ...
        WHERE a.id_empresa = e.id_empresa
          AND a.id_tipo_accion_personal = 9
          AND LOWER(a.nombre_articulo_nomina) = LOWER('Articulo de nomina Bonificacion')
      )
    `);
    ...
    await queryRunner.query(`
      ...
      SELECT
        ...
        NULL,
        NULL,
        'Movimiento de bonificacion por monto fijo.',
        1,
        '10000',
        '0',
        ...
        0
      FROM nom_articulos_nomina a
      WHERE a.id_tipo_accion_personal = 9
        AND a.es_inactivo = 0
        AND NOT EXISTS (
          SELECT 1
          FROM nom_movimientos_nomina m
          WHERE m.id_empresa_movimiento_nomina = a.id_empresa
            AND m.id_tipo_accion_personal_movimiento_nomina = 9
            AND LOWER(m.nombre_movimiento_nomina) = LOWER('Bonificacion Fija')
        )
    `);
    ...
    await queryRunner.query(`
      ...
      SELECT
        ...
        NULL,
        NULL,
        'Movimiento de bonificacion por porcentaje sobre salario base del periodo.',
        0,
        '0',
        '10',
      ...
      WHERE a.id_tipo_accion_personal = 9
        ...
        AND NOT EXISTS (
          ...
          WHERE m.id_empresa_movimiento_nomina = a.id_empresa
            AND m.id_tipo_accion_personal_movimiento_nomina = 9
            AND LOWER(m.nombre_movimiento_nomina) = LOWER('Bonificacion Porcentaje')
        )
    `);
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      ...
      WHERE id_tipo_accion_personal_movimiento_nomina = 9
        AND LOWER(nombre_movimiento_nomina) IN (
          LOWER('Bonificacion Fija'),
          LOWER('Bonificacion Porcentaje')
        )
    `);

    await queryRunner.query(`
      DELETE FROM nom_articulos_nomina
      WHERE id_tipo_accion_personal = 9
        AND LOWER(nombre_articulo_nomina) = LOWER('Articulo de nomina Bonificacion')
    `);
  }
}
