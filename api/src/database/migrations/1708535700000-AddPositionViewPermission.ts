...
export class AddPositionViewPermission1708535700000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      ...
      VALUES
        ('position:view', 'Ver/listar puestos', 'Permite ver y listar puestos organizacionales', 'position', 1, '${now}')
    ...
    await queryRunner.query(`
      ...
      INNER JOIN sys_permisos p
        ON p.codigo_permiso = 'position:view'
    ...
    await queryRunner.query(`
      ...
      FROM (
        ...
        WHERE p.codigo_permiso = 'config:puestos'
      ) legacy
      INNER JOIN sys_permisos pnew
        ON pnew.codigo_permiso = 'position:view'
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      ...
      FROM sys_rol_permiso rp
      INNER JOIN sys_permisos p ON p.id_permiso = rp.id_permiso
      WHERE p.codigo_permiso = 'position:view'
    `);

    await queryRunner.query(`
      DELETE FROM sys_permisos
      WHERE codigo_permiso = 'position:view'
    `);
  }
}
