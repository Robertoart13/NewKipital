  ...
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      CREATE TABLE IF NOT EXISTS acc_descuentos_lineas (
        id_linea_descuento INT NOT NULL AUTO_INCREMENT,
        ...
        PRIMARY KEY (id_linea_descuento),
        INDEX IDX_dsc_linea_accion (id_accion),
        INDEX IDX_dsc_linea_cuota (id_cuota),
        INDEX IDX_dsc_linea_empresa (id_empresa),
        INDEX IDX_dsc_linea_empleado (id_empleado),
        INDEX IDX_dsc_linea_calendario (id_calendario_nomina),
        INDEX IDX_dsc_linea_movimiento (id_movimiento_nomina),
        CONSTRAINT FK_dsc_linea_accion FOREIGN KEY (id_accion)
          REFERENCES acc_acciones_personal(id_accion),
        CONSTRAINT FK_dsc_linea_cuota FOREIGN KEY (id_cuota)
          REFERENCES acc_cuotas_accion(id_cuota),
        CONSTRAINT FK_dsc_linea_empresa FOREIGN KEY (id_empresa)
          REFERENCES sys_empresas(id_empresa),
        CONSTRAINT FK_dsc_linea_empleado FOREIGN KEY (id_empleado)
          REFERENCES sys_empleados(id_empleado),
        CONSTRAINT FK_dsc_linea_calendario FOREIGN KEY (id_calendario_nomina)
          REFERENCES nom_calendarios_nomina(id_calendario_nomina),
        CONSTRAINT FK_dsc_linea_movimiento FOREIGN KEY (id_movimiento_nomina)
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      DROP TABLE IF EXISTS acc_descuentos_lineas
    `);
  }
}
