-- Seed idempotente de articulo y movimientos para pruebas UX de Descuentos
-- Empresa objetivo: id_empresa = 1 (Rocca Master Company)
-- Tipo de accion personal: 6 (DESCUENTO)
...
SELECT
  1,
  'QA Articulo Nomina Descuentos UX',
  'Articulo de prueba para UX Descuentos',
  6,
  2,
  1,
  NULL,
...
WHERE NOT EXISTS (
  ...
  WHERE id_empresa = 1
    AND nombre_articulo_nomina = 'QA Articulo Nomina Descuentos UX'
    AND id_tipo_accion_personal = 6
);
...
SELECT
  ...
  '12000',
  '0',
  'Monto fijo: 12000 x cantidad',
  0
FROM nom_articulos_nomina a
WHERE a.id_empresa = 1
  AND a.nombre_articulo_nomina = 'QA Articulo Nomina Descuentos UX'
  AND NOT EXISTS (
    SELECT 1
    FROM nom_movimientos_nomina m
    WHERE m.id_empresa_movimiento_nomina = 1
      AND m.nombre_movimiento_nomina = 'QA Descuento Monto Fijo 12000'
...
SELECT
  ...
  '0',
  '8',
  'Salario base ajustado por periodo x 8% x cantidad',
  0
FROM nom_articulos_nomina a
WHERE a.id_empresa = 1
  AND a.nombre_articulo_nomina = 'QA Articulo Nomina Descuentos UX'
  AND NOT EXISTS (
    ...
    FROM nom_movimientos_nomina m
    WHERE m.id_empresa_movimiento_nomina = 1
      AND m.nombre_movimiento_nomina = 'QA Descuento Porcentaje 8'
      AND m.id_tipo_accion_personal_movimiento_nomina = 6
  )
LIMIT 1;
