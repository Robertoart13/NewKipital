# Manual Maestro de Usuario - KPITAL 360

## Entrada principal del modulo Ayuda
Este archivo es la portada operativa del usuario final en `Ayuda` (`/docs`).
Para manual completo enterprise, abrir:

- [Manual de Usuario Enterprise - KPITAL 360](./14-MANUAL-USUARIO-ENTERPRISE-KPITAL360.md)

## Ruta recomendada de lectura
1. [Manual Enterprise completo](./14-MANUAL-USUARIO-ENTERPRISE-KPITAL360.md)
2. [Mapa de menus y rutas](./07-MAPA-MENUS-Y-RUTAS.md)
3. [Planilla operativa](./05-PLANILLA-OPERATIVA.md)
4. [Acciones de personal](./06-ACCIONES-PERSONAL-OPERATIVO.md)
5. [Usuarios, roles y permisos](./10-USUARIOS-ROLES-PERMISOS.md)

## Atajos rapidos
- Crear empresa: [Empresas](./01-EMPRESAS.md)
- Crear empleado: [Empleados](./02-EMPLEADOS.md)
- Configurar articulos: [Articulos de nomina](./03-ARTICULOS-NOMINA.md)
- Configurar movimientos: [Movimientos de nomina](./12-MOVIMIENTOS-NOMINA.md)
- Operar planilla regular: [Planilla operativa](./05-PLANILLA-OPERATIVA.md)
- Revisar cierre de planillas (verificadas/aplicadas): [Planilla operativa](./05-PLANILLA-OPERATIVA.md#lista-de-planillas-aplicadas-finalizacion-de-proceso)
- Ajustes de seguridad: [Usuarios, roles y permisos](./10-USUARIOS-ROLES-PERMISOS.md)

## Regla de oro de operacion
Si una accion no se puede ejecutar, revisar en este orden:
1. Empresa activa.
2. Permiso del rol.
3. Estado del registro.
4. Regla de negocio del modulo.

## Ver tambien
- [Indice general de documentacion](../00-INDICE-CONSOLIDACION.md)
- [Manual tecnico](../14-manual-tecnico/00-STACK-Y-ARQUITECTURA.md)
- [Diagramas de procesos operativos](./19-DIAGRAMAS-PROCESOS-OPERATIVOS.md)
- [Evidencia de capturas de pantalla](./20-EVIDENCIA-CAPTURAS-PANTALLA.md)
