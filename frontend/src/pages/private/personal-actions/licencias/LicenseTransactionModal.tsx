...
import type { LicenseType, PersonalActionAuditTrailItem } from '../../../../api/personalActions';
...
export interface LicenseTransactionLine {
  key: string;
  payrollId?: number;
  payrollLabel?: string;
...
export interface LicenseFormDraft {
  idEmpresa: number;
  idEmpleado: number;
  observacion?: string;
  lines: LicenseTransactionLine[];
}

interface LicenseTransactionModalProps {
  open: boolean;
  mode: 'create' | 'edit';
  title: string;
...
const LICENSE_TYPE_OPTIONS: Array<{ value: LicenseType; label: string }> = [
  ...
  { value: 'duelo', label: 'Licencia por duelo o fallecimiento de familiar cercano' },
  { value: 'matrimonio', label: 'Licencia por matrimonio' },
  { value: 'estudios', label: 'Licencia para estudios' },
  { value: 'lactancia', label: 'Licencia por lactancia (sector público)' },
  { value: 'cuidado_familiar', label: 'Licencia para cuidado de familiar enfermo' },
  { value: 'permiso_con_goce', label: 'Permiso especial con goce de salario' },
  { value: 'permiso_sin_goce', label: 'Permiso especial sin goce de salario' },
  { value: 'citacion_judicial', label: 'Licencia por citación judicial' },
  { value: 'votacion', label: 'Permiso para ejercer el voto' },
  { value: 'donacion_sangre', label: 'Licencia por donación de sangre (usualmente 1 día)' },
  { value: 'licencia_sindical', label: 'Licencia para actividades sindicales (según convenio)' },
  {
    value: 'licencia_especial_empresa',
    label: 'Otras licencias especiales según política interna',
  },
];

function buildEmptyLine(): LicenseTransactionLine {
  return {
...
export function LicenseTransactionModal({
  open,
  mode,
  title,
  companies,
  ...
  const filteredMovements = useMemo(() => {
    ...
    if (actionTypeIdForLicense) {
      list = list.filter((movement) => movement.idTipoAccionPersonal === actionTypeIdForLicense);
    }
    ...
    return list;
  }, [movements, selectedCompanyIdNum, actionTypeIdForLicense, lines]);
    ...
    modal.confirm({
      ...
      content:
        mode === 'create'
          ...
          : '¿Está seguro de guardar los cambios de esta licencia?',
          ...
          <div style={{ flexShrink: 0 }}>
            ...
            {mode !== 'edit' || activeTab === 'info' ? (
                  ...
                  {selectedCompanyId && selectedEmployeeId ? (
                    ...
                    >
                      <Result
                        ...
                        subTitle="Seleccione empresa y empleado para agregar líneas de licencia por planilla."
                      />
                    </div>
                  )}
                </Col>
              ...
              <div className={sharedStyles.historicoSection}>
                <p className={sharedStyles.sectionTitle}>Historial de cambios de la licencia</p>
                ...
                <Table<PersonalActionAuditTrailItem>
                  ...
                  locale={{ emptyText: 'No hay registros de bitacora para esta licencia.' }}
                />
              </div>
            )}
          </div>
          ...
          <div style={{ flexShrink: 0 }}>
            <div className={sharedStyles.companyModalFooter}>
              ...
              {!readOnly ? (
                ...
                >
                  {mode === 'create' ? 'Crear licencia' : 'Guardar cambios'}
                </Button>
              ) : null}
            </div>
          </div>
