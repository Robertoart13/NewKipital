...
import {
  DisabilityTransactionModal,
  type DisabilityFormDraft,
  type DisabilityTransactionLine,
} from './IncapacityTransactionModal';
...
const NEXT_STATE_ACTION_CONFIG: Partial<Record<number, NextStateActionConfig>> = {
  ...
  3: {
    label: 'Aprobar',
    requiredPermission: 'approve',
    confirmText: 'La incapacidad quedara APROBADA y lista para proceso operativo. Desea continuar?',
    successText: 'La incapacidad fue aprobada correctamente.',
    deniedText: 'No tiene permiso para aprobar incapacidades.',
  },
};
...
interface DisabilityUiRow extends PersonalActionListItem {
  employeeLabel?: string;
}

function isDisabilityEditableState(estado: number): boolean {
...
function getPaneValue(
  ...
  if (key === 'remuneracion') return row.remuneracionResumen === 'SI' ? 'Si' : row.remuneracionResumen === 'NO' ? 'No' : row.remuneracionResumen === 'MIXTA' ? 'Mixta' : '--';
...
function createDraftFromDisabilityDetail(detail: DisabilityDetailItem): DisabilityFormDraft {
  const lines: DisabilityTransactionLine[] =
    detail.lines?.length > 0
      ? detail.lines.map((line) => ({
        ...
        monto: line.monto,
        montoIns: line.montoIns,
        montoPatrono: line.montoPatrono,
...
export function IncapacitiesPage() {
  ...
  const [rows, setRows] = useState<DisabilityUiRow[]>([]);
  ...
  const [editingRow, setEditingRow] = useState<DisabilityUiRow | null>(null);
  const [editingDraft, setEditingDraft] = useState<DisabilityFormDraft | undefined>(undefined);
  ...
  const openEditModal = useCallback(async (row: DisabilityUiRow) => {
    const key = `Disability-edit-load-${row.id}`;
    setEditingRow(row);
    setEditingDraft({
    ...
    setAuditTrail([]);
    setOpenModal(true);
    message.loading({ content: 'Cargando detalle de incapacidad...', key, duration: 0 });
    try {
      const detail = await fetchDisabilityDetail(row.id);
      setEditingDraft(createDraftFromDisabilityDetail(detail));
      message.destroy(key);
    } catch (error) {
      message.error({
  ...
  const loadDisabilityAuditTrail = useCallback(async (id: number) => {
    setLoadingAuditTrail(true);
    try {
      const rowsAudit = await fetchDisabilityAuditTrail(id, 200);
  ...
  const loadEditingDisabilityAuditTrail = useCallback(async () => {
  ...
  const loadCatalogs = useCallback(async () => {
    if (!companyId) {
      ...
      setMovements([]);
      setPayPeriods([]);
      setDisabilityActionTypeId(undefined);
  ...
  const columns: ColumnsType<DisabilityUiRow> = useMemo(() => [
    ...
    },
    {
      ...
      width: 140,
      render: (value: DisabilityUiRow['remuneracionResumen']) => {
        if (value === 'SI') return <Tag color="green">Si</Tag>;
      ...
      render: (_, row) => {
        ...
        const onInvalidate = (e: MouseEvent<HTMLElement>) => {
          ...
          modal.confirm({
            ...
            onOk: async () => {
              const key = `Disability-invalidate-${row.id}`;
              message.loading({ content: 'Invalidando incapacidad...', key, duration: 0 });
              try {
                await invalidateDisability(row.id);
                message.success({ content: 'incapacidad invalidada correctamente.', key });
                await loadRows();
              } catch (error) {
                message.error({
            ...
            <Button
              ...
              onClick={(e) => {
                ...
                modal.confirm({
                  ...
                  cancelButtonProps: { className: styles.companyConfirmCancel },
                  onOk: async () => {
                    const key = `Disability-advance-${row.id}`;
                    message.loading({ content: 'Actualizando estado...', key, duration: 0 });
                    try {
                      await advanceDisabilityState(row.id);
  ...
  return (
    <div className={styles.pageWrapper}>
      <div className={styles.pageHeader}>
        <div className={styles.pageHeaderLeft}>
          ...
          <div className={styles.pageTitleBlock}>
            ...
            <p className={styles.pageSubtitle}>Gestione incapacidades por empresa con lineas de transaccion por periodo</p>
          </div>
        </div>
      </div>
          ...
          <Flex align="center" justify="space-between" wrap="wrap" gap={12} className={styles.registrosHeader}>
            <Flex align="center" gap={12} wrap="wrap">
              <Flex align="center" gap={8}>
                ...
                <h3 className={styles.registrosTitle}>Registros de incapacidades</h3>
              </Flex>
      ...
      <DisabilityTransactionModal
        ...
        movements={movements}
        actionTypeIdForDisability={disabilityActionTypeId}
        ...
        readOnly={mode === 'edit' && !!editingRow && !isDisabilityEditableState(editingRow.estado)}
        readOnlyMessage={
          mode === 'edit' && !!editingRow && !isDisabilityEditableState(editingRow.estado)
            ? `Esta incapacidad esta en estado "${ESTADO_LABEL[editingRow.estado]?.text ?? editingRow.estado}" y solo se puede consultar.`
            : undefined
        }
        ...
        onSubmit={async (draft) => {
          if (mode === 'edit' && editingRow && !isDisabilityEditableState(editingRow.estado)) {
            message.warning('Esta incapacidad esta en modo solo lectura.');
            return;
          }
          const payload = mapDraftToPayload(draft);
          const loadingKey = 'Disability-save';
          message.loading({ content: 'Guardando incapacidad...', key: loadingKey, duration: 0 });
          try {
            let totalCreated = 1;
            if (mode === 'edit' && editingRow) {
              await updateDisability(editingRow.id, payload);
            } else {
              const created = await createDisability(payload);
              totalCreated =
            ...
            message.success({
              content:
                mode === 'edit'
                  ? 'incapacidad actualizada correctamente.'
                  : totalCreated > 1
                    ? `Se crearon ${totalCreated} incapacidades (una por periodo).`
