...
export function VacationsPage() {
  ...
  const openEditModal = useCallback(
    async (row: VacationUiRow) => {
      ...
      setModalEmployees([]);
      setOpenModal(true);
      void fetchAbsenceEmployeesCatalog(row.idEmpresa)
        .then(setModalEmployees)
        .catch(() => setModalEmployees([]));
  ...
  const loadVacationAuditTrail = useCallback(
    async (id: number) => {
        ...
        <div className={styles.pageHeaderLeft}>
          ...
          <div className={styles.pageTitleBlock}>
            ...
            <p className={styles.pageSubtitle}>
              Gestione acciones de vacaciones por colaborador con calendario de días
            </p>
          </div>
        </div>
      </div>
