...
import { RetentionTransactionModal,
  type RetentionFormDraft,
  type RetentionTransactionLine,
} from './RetentionTransactionModal';
...
const NEXT_STATE_ACTION_CONFIG: Partial<Record<number, NextStateActionConfig>> = {
  1: {
    ...
    successText: 'La retencion fue enviada a Supervisor.',
  ...
  2: {
    ...
    successText: 'La retencion fue enviada a RRHH.',
    deniedText: 'No tiene permiso para enviar a RRHH.',
  },
  3: {
    label: 'Aprobar',
    requiredPermission: 'approve',
    confirmText: 'La retencion quedara APROBADA y lista para proceso operativo. Desea continuar?',
    successText: 'La retencion fue aprobada correctamente.',
    deniedText: 'No tiene permiso para aprobar retenciones.',
  },
};
...
interface RetentionUiRow extends PersonalActionListItem {
  employeeLabel?: string;
}

function isRetentionEditableState(estado: number): boolean {
...
export function RetentionsPage() {
  ...
  const canCreate = useAppSelector((state) => hasPermission(state, 'hr-action-retenciones:create'));
  ...
  const canView = useAppSelector((state) =>
    hasPermission(state, 'hr-action-retenciones:view') ||
  ...
  const [rows, setRows] = useState<RetentionUiRow[]>([]);
  ...
  const [retentionActionTypeId, setRetentionActionTypeId] = useState<number | undefined>(undefined);
  ...
  const [editingRow, setEditingRow] = useState<RetentionUiRow | null>(null);
  const [editingDraft, setEditingDraft] = useState<RetentionFormDraft | undefined>(undefined);
  ...
  const openEditModal = useCallback(async (row: RetentionUiRow) => {
    const key = `Retention-edit-load-${row.id}`;
    setEditingRow(row);
    setEditingDraft({
    ...
    setAuditTrail([]);
    setOpenModal(true);
    message.loading({ content: 'Cargando detalle de retencion...', key, duration: 0 });
    try {
      const detail = await fetchRetentionDetail(row.id);
      setEditingDraft(createDraftFromRetentionDetail(detail));
      message.destroy(key);
    } catch (error) {
      message.error({
  ...
  const loadRetentionAuditTrail = useCallback(async (id: number) => {
    setLoadingAuditTrail(true);
    try {
      const rowsAudit = await fetchRetentionAuditTrail(id, 200);
  ...
  const loadEditingRetentionAuditTrail = useCallback(async () => {
    if (!editingRow?.id) return;
    await loadRetentionAuditTrail(editingRow.id);
  }, [editingRow?.id, loadRetentionAuditTrail]);
  ...
  const loadCatalogs = useCallback(async () => {
    if (!companyId) {
      ...
      setMovements([]);
      setPayPeriods([]);
      setRetentionActionTypeId(undefined);
    ...
    try {
      const [employeesResp, movementsResp, payPeriodsResp] = await Promise.all([
        ...
        fetchAbsenceMovementsCatalog(companyId, 5),
      ...
      render: (_, row) => {
        ...
        const onInvalidate = (e: MouseEvent<HTMLElement>) => {
          ...
          modal.confirm({
            ...
            onOk: async () => {
              const key = `Retention-invalidate-${row.id}`;
              message.loading({ content: 'Invalidando retencion...', key, duration: 0 });
              try {
                await invalidateRetention(row.id);
                message.success({ content: 'retencion invalidada correctamente.', key });
                await loadRows();
              } catch (error) {
                message.error({
            ...
            <Button
              ...
              onClick={(e) => {
                ...
                modal.confirm({
                  ...
                  cancelButtonProps: { className: styles.companyConfirmCancel },
                  onOk: async () => {
                    const key = `Retention-advance-${row.id}`;
                    message.loading({ content: 'Actualizando estado...', key, duration: 0 });
                    try {
                      await advanceRetentionState(row.id);
  ...
  return (
    <div className={styles.pageWrapper}>
      <div className={styles.pageHeader}>
        <div className={styles.pageHeaderLeft}>
          ...
          <div className={styles.pageTitleBlock}>
            ...
            <p className={styles.pageSubtitle}>Gestione retenciones por empresa con líneas de transacción por período</p>
          </div>
        </div>
      </div>
          ...
          <Flex align="center" justify="space-between" wrap="wrap" gap={12} className={styles.registrosHeader}>
            <Flex align="center" gap={12} wrap="wrap">
              <Flex align="center" gap={8}>
                ...
                <h3 className={styles.registrosTitle}>Registros de retenciones</h3>
              </Flex>
      ...
      <RetentionTransactionModal
        ...
        movements={movements}
        actionTypeIdForRetention={retentionActionTypeId}
        ...
        readOnly={mode === 'edit' && !!editingRow && !isRetentionEditableState(editingRow.estado)}
        readOnlyMessage={
          mode === 'edit' && !!editingRow && !isRetentionEditableState(editingRow.estado)
            ? `Esta retencion esta en estado "${ESTADO_LABEL[editingRow.estado]?.text ?? editingRow.estado}" y solo se puede consultar.`
            : undefined
        }
        ...
        onSubmit={async (draft) => {
          if (mode === 'edit' && editingRow && !isRetentionEditableState(editingRow.estado)) {
            message.warning('Esta retencion esta en modo solo lectura.');
            return;
          }
          const payload = mapDraftToPayload(draft);
          const loadingKey = 'Retention-save';
          message.loading({ content: 'Guardando retencion...', key: loadingKey, duration: 0 });
          try {
            let totalCreated = 1;
            if (mode === 'edit' && editingRow) {
              await updateRetention(editingRow.id, payload);
            } else {
              const created = await createRetention(payload);
              totalCreated =
            ...
            message.success({
              content:
                mode === 'edit'
                  ? 'retencion actualizada correctamente.'
                  : totalCreated > 1
                    ? `Se crearon ${totalCreated} retenciones (una por periodo).`
                    : 'retencion creada correctamente.',
              key: loadingKey,
            });
            setOpenModal(false);
            setEditingDraft(undefined);
            ...
            message.error({
              content:
                error instanceof Error
                  ? error.message
                  : 'No se pudo guardar la retencion.',
              key: loadingKey,
              duration: 5,
            });
          }
