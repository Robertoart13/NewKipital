...
export interface RetentionTransactionLine {
  key: string;
  payrollId?: number;
  payrollLabel?: string;
...
export interface RetentionFormDraft {
  idEmpresa: number;
  idEmpleado: number;
  observacion?: string;
  lines: RetentionTransactionLine[];
}

interface RetentionTransactionModalProps {
  open: boolean;
  mode: 'create' | 'edit';
  title: string;
  ...
  actionTypeIdForRetention?: number;
  ...
  initialDraft?: RetentionFormDraft;
  onCancel: () => void;
  onSubmit: (payload: RetentionFormDraft) => Promise<void> | void;
}
...
function buildEmptyLine(): RetentionTransactionLine {
  return {
...
export function RetentionTransactionModal({
  open,
  mode,
  title,
  companies,
  ...
  onCancel,
  onSubmit,
}: RetentionTransactionModalProps) {
  ...
  const handleAccept = async () => {
    ...
    modal.confirm({
      title:
        mode === 'create'
          ? 'Confirmar creación de retención'
          : 'Confirmar actualización de retención',
      content:
        mode === 'create'
          ...
          <div style={{ flexShrink: 0 }}>
            {readOnly ? (
              <Alert
                ...
                title={
                  readOnlyMessage ??
                  'Esta retención está en modo solo lectura por su estado actual.'
                }
            ...
            {mode !== 'edit' || activeTab === 'info' ? (
              ...
              <div className={sharedStyles.historicoSection}>
                <p className={sharedStyles.sectionTitle}>Historial de cambios de la retencion</p>
          ...
          <div style={{ flexShrink: 0 }}>
            <div className={sharedStyles.companyModalFooter}>
              ...
              {!readOnly ? (
                ...
                >
                  {mode === 'create' ? 'Crear retencion' : 'Guardar cambios'}
                </Button>
              ) : null}
            </div>
          </div>
