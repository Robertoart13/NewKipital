...
import { HoursExtraTransactionModal,
  type OvertimeFormDraft,
  type OvertimeTransactionLine,
} from './HoursExtraTransactionModal';
...
const NEXT_STATE_ACTION_CONFIG: Partial<Record<number, NextStateActionConfig>> = {
  ...
  3: {
    ...
    deniedText: 'No tiene permiso para aprobar horas extra.',
  },
};
...
interface OvertimeUiRow extends PersonalActionListItem {
  employeeLabel?: string;
}

function isOvertimeEditableState(estado: number): boolean {
...
export function HoursExtrasPage() {
  ...
  const canView = useAppSelector((state) =>
    hasPermission(state, 'hr-action-horas-extras:view') ||
  ...
  const [rows, setRows] = useState<OvertimeUiRow[]>([]);
  ...
  const [overtimeActionTypeId, setOvertimeActionTypeId] = useState<number | undefined>(11);
  ...
  const [editingRow, setEditingRow] = useState<OvertimeUiRow | null>(null);
  const [editingDraft, setEditingDraft] = useState<OvertimeFormDraft | undefined>(undefined);
  ...
  const openEditModal = useCallback(async (row: OvertimeUiRow) => {
    const key = `Overtime-edit-load-${row.id}`;
    setEditingRow(row);
    setEditingDraft({
    ...
    setAuditTrail([]);
    setOpenModal(true);
    message.loading({ content: 'Cargando detalle de hora extra...', key, duration: 0 });
    try {
      const detail = await fetchOvertimeDetail(row.id);
      setEditingDraft(createDraftFromOvertimeDetail(detail));
      message.destroy(key);
    } catch (error) {
      message.error({
  ...
  const loadOvertimeAuditTrail = useCallback(async (id: number) => {
    setLoadingAuditTrail(true);
    try {
      const rowsAudit = await fetchOvertimeAuditTrail(id, 200);
  ...
  const loadEditingOvertimeAuditTrail = useCallback(async () => {
    if (!editingRow?.id) return;
    await loadOvertimeAuditTrail(editingRow.id);
  }, [editingRow?.id, loadOvertimeAuditTrail]);
  ...
  const loadCatalogs = useCallback(async () => {
    ...
    try {
      const [employeesResp, movementsResp, payPeriodsResp] = await Promise.all([
        ...
        fetchAbsenceMovementsCatalog(companyId, 11),
      ...
      setMovements([]);
      setPayPeriods([]);
      setOvertimeActionTypeId(undefined);
    } finally {
      setLoadingEmployees(false);
      setLoadingMovements(false);
    }
      ...
      render: (_, row) => {
        ...
        const onInvalidate = (e: MouseEvent<HTMLElement>) => {
          ...
          modal.confirm({
            ...
            onOk: async () => {
              const key = `Overtime-invalidate-${row.id}`;
              message.loading({ content: 'Invalidando hora extra...', key, duration: 0 });
              try {
                await invalidateOvertime(row.id);
                message.success({ content: 'hora extra invalidada correctamente.', key });
                await loadRows();
              } catch (error) {
                message.error({
            ...
            <Button
              ...
              onClick={(e) => {
                ...
                modal.confirm({
                  ...
                  cancelButtonProps: { className: styles.companyConfirmCancel },
                  onOk: async () => {
                    const key = `Overtime-advance-${row.id}`;
                    message.loading({ content: 'Actualizando estado...', key, duration: 0 });
                    try {
                      await advanceOvertimeState(row.id);
  ...
  const mapDraftToPayload = (draft: OvertimeFormDraft) => ({
  ...
  return (
    <div className={styles.pageWrapper}>
      <div className={styles.pageHeader}>
        <div className={styles.pageHeaderLeft}>
          ...
          <div className={styles.pageTitleBlock}>
            ...
            <p className={styles.pageSubtitle}>Gestione horas extra por empresa con líneas de transacción por periodo</p>
          </div>
        </div>
      </div>
          ...
          <Flex align="center" justify="space-between" wrap="wrap" gap={12} className={styles.registrosHeader}>
            <Flex align="center" gap={12} wrap="wrap">
              <Flex align="center" gap={8}>
                ...
                <h3 className={styles.registrosTitle}>Registros de horas extra</h3>
              </Flex>
      ...
      <HoursExtraTransactionModal
        ...
        movements={movements}
        actionTypeIdForOvertime={overtimeActionTypeId}
        ...
        readOnly={mode === 'edit' && !!editingRow && !isOvertimeEditableState(editingRow.estado)}
        readOnlyMessage={
          mode === 'edit' && !!editingRow && !isOvertimeEditableState(editingRow.estado)
            ? `Esta hora extra esta en estado "${ESTADO_LABEL[editingRow.estado]?.text ?? editingRow.estado}" y solo se puede consultar.`
            : undefined
        }
        ...
        onLoadAuditTrail={
          mode === 'edit' && editingRow ? loadEditingOvertimeAuditTrail : undefined
        }
        ...
        onSubmit={async (draft) => {
          ...
          const loadingKey = 'Overtime-save';
          message.loading({ content: 'Guardando hora extra...', key: loadingKey, duration: 0 });
          try {
            let totalCreated = 1;
            if (mode === 'edit' && editingRow) {
              await updateOvertime(editingRow.id, payload);
            } else {
            ...
            message.success({
              content:
                mode === 'edit'
                  ...
                  : totalCreated > 1
                    ? `Se crearon ${totalCreated} horas extra (una por periodo).`
