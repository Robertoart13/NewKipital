...
function getPaneValue(row: AbsenceUiRow, key: PaneKey, companies: Array<{ id: number; nombre: string }>): string {
  if (key === 'empresa') {
...
export function AbsencesPage() {
  ...
  const canView = useAppSelector(
    (state) => hasPermission(state, 'hr-action-ausencias:view') || hasPermission(state, 'hr_action:view'),
  );
  ...
  const columns: ColumnsType<AbsenceUiRow> = useSortableColumns(
    () => [
      {
        title: 'EMPRESA',
        key: 'empresa',
