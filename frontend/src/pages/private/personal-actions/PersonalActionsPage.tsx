...
export function PersonalActionsPage({
  ...
  pageSubtitle = 'Gestione acciones de personal por empresa para integraciÃ³n con planilla',
  fixedTipoAccion,
  ...
  const columns: ColumnsType<PersonalActionListItem> = useSortableColumns(
    () => [
            ...
            <Button
              ...
              onClick={() => {
                modal.confirm({
                  ...
                  content: `Se aprobarÃ¡ la acciÃ³n #${row.id}.`,
                  okText: 'Aprobar',
                  cancelText: 'Cancelar',
                  onOk: async () => {
                    try {
                      ...
                      message.error(error instanceof Error ? error.message : 'No se pudo aprobar la acciÃ³n.');
                    }
                  },
                });
              }}
            ...
            <Button
              ...
              onClick={() => {
                modal.confirm({
                  ...
                  content: `Se rechazarÃ¡ la acciÃ³n #${row.id}.`,
                  okText: 'Rechazar',
                  cancelText: 'Cancelar',
                  onOk: async () => {
                    try {
                      ...
                      message.error(error instanceof Error ? error.message : 'No se pudo rechazar la acciÃ³n.');
                    }
                  },
                });
              }}
  ...
  const handleSubmit = async () => {
    if (!companyId) {
      message.error('Seleccione una empresa antes de crear la acciÃ³n.');
      return;
    }
            ...
            <Flex align="center" gap={16}>
              ...
              <div>
                ...
                <p className={styles.gestionDesc}>
                  Listado, aprobaciÃ³n y rechazo de acciones segÃºn el flujo de RRHH y Supervisor
                </p>
              </div>
            </Flex>
            <Button
        ...
        <Form form={form} layout="vertical">
          <Flex gap={12} wrap="wrap">
            ...
            >
              <Select
                ...
                options={sortEmployeesByDisplayName(employees).map((employee) => ({
                  value: employee.id,
          ...
          <Flex gap={12} wrap="wrap">
            <Form.Item
              name="tipoAccion"
              label="Tipo Accion"
              rules={[{ required: true, message: 'Digite o seleccione el tipo de acciÃ³n' }]}
          ...
          {selectedHelper && (
            <Card size="small" style={{ background: '#f8fafc', borderColor: '#dbe5ef' }}>
              ...
              <div>Moneda: {selectedHelper.moneda ?? '?'}</div>
              <div>Nota: se usa solo como apoyo visual, no enlaza la acciÃ³n directamente al run.</div>
            </Card>
          )}
        </Form>
      </Modal>
