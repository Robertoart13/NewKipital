...
const NEXT_STATE_ACTION_CONFIG: Partial<Record<number, NextStateActionConfig>> = {
  1: {
    ...
    successText: 'El descuento fue enviado a Supervisor.',
  ...
  2: {
    ...
    successText: 'El descuento fue enviado a RRHH.',
    deniedText: 'No tiene permiso para enviar a RRHH.',
  },
  3: {
    label: 'Aprobar',
    requiredPermission: 'approve',
    confirmText: 'El descuento quedara APROBADO y listo para proceso operativo. Desea continuar?',
    successText: 'El descuento fue aprobado correctamente.',
    deniedText: 'No tiene permiso para aprobar descuentos.',
  },
};
...
interface DiscountUiRow extends PersonalActionListItem {
  employeeLabel?: string;
}

function isDiscountEditableState(estado: number): boolean {
...
export function DiscountsPage() {
  ...
  const canView = useAppSelector(
    (state) => hasPermission(state, 'hr-action-descuentos:view') || hasPermission(state, 'hr_action:view'),
  );
  ...
  const [rows, setRows] = useState<DiscountUiRow[]>([]);
  ...
  const [DiscountActionTypeId, setDiscountActionTypeId] = useState<number | undefined>(undefined);
  ...
  const [editingRow, setEditingRow] = useState<DiscountUiRow | null>(null);
  const [editingDraft, setEditingDraft] = useState<DiscountFormDraft | undefined>(undefined);
  ...
  const loadEditingDiscountAuditTrail = useCallback(async () => {
    ...
    await loadDiscountAuditTrail(editingRow.id);
  ...
  const loadCatalogs = useCallback(async () => {
    ...
    if (!targetCompanyId) {
      ...
      setMovements([]);
      setPayPeriods([]);
      setDiscountActionTypeId(undefined);
  ...
  return (
    <div className={styles.pageWrapper}>
          ...
          <Flex align="center" justify="space-between" wrap="wrap" gap={12} className={styles.registrosHeader}>
            <Flex align="center" gap={12} wrap="wrap">
              <Flex align="center" gap={8}>
                ...
                <h3 className={styles.registrosTitle}>Registros de descuentos</h3>
              </Flex>
      ...
      <DiscountTransactionModal
        ...
        movements={movements}
        actionTypeIdForDiscount={DiscountActionTypeId}
        ...
        readOnly={mode === 'edit' && !!editingRow && !isDiscountEditableState(editingRow.estado)}
        readOnlyMessage={
          mode === 'edit' && !!editingRow && !isDiscountEditableState(editingRow.estado)
            ? `Este descuento esta en estado "${ESTADO_LABEL[editingRow.estado]?.text ?? editingRow.estado}" y solo se puede consultar.`
            : undefined
        }
        ...
        onSubmit={async (draft) => {
          if (mode === 'edit' && editingRow && !isDiscountEditableState(editingRow.estado)) {
            message.warning('Este descuento esta en modo solo lectura.');
            return;
          }
          const payload = mapDraftToPayload(draft);
          const loadingKey = 'Discount-save';
          message.loading({ content: 'Guardando descuento...', key: loadingKey, duration: 0 });
          try {
            let totalCreated = 1;
            ...
            message.success({
              content:
                mode === 'edit'
                  ...
                  : totalCreated > 1
                    ? `Se crearon ${totalCreated} descuentos (una por periodo).`
