...
export interface DiscountTransactionLine {
  key: string;
  payrollId?: number;
  payrollLabel?: string;
...
export interface DiscountFormDraft {
  idEmpresa: number;
  idEmpleado: number;
  observacion?: string;
  lines: DiscountTransactionLine[];
}

interface DiscountTransactionModalProps {
  open: boolean;
  mode: 'create' | 'edit';
  title: string;
  ...
  actionTypeIdForDiscount?: number;
  ...
  onCancel: () => void;
  onSubmit: (payload: DiscountFormDraft) => Promise<void> | void;
}
...
function buildEmptyLine(): DiscountTransactionLine {
  return {
...
export function DiscountTransactionModal({
  open,
  mode,
  title,
  companies,
  ...
  const handleAccept = async () => {
    ...
    if (lines.length === 0 || !lines.every(isLineComplete)) {
      message.error('Complete todas las lineas antes de crear/guardar el descuento.');
      return;
    }
    ...
    modal.confirm({
      title: mode === 'create' ? 'Confirmar creacion de descuento' : 'Confirmar actualizacion de descuento',
      content:
        mode === 'create'
          ? 'Esta seguro de crear este descuento con las lineas capturadas?'
          : 'Esta seguro de guardar los cambios de este descuento?',
          ...
          <div style={{ flexShrink: 0 }}>
            {readOnly ? (
              <Alert
                type="warning"
                showIcon
                title={readOnlyMessage ?? 'Este descuento esta en modo solo lectura por su estado actual.'}
                                    ...
                                    <div ref={index === lines.length - 1 ? lastLineRef : undefined}>
                                      <Space orientation="vertical" size={16} style={{ width: '100%' }}>
                                        <Row gutter={[16, 12]}>
                                          ...
                                          <Col xs={24} md={12} lg={8}>
                                            <div className={sharedStyles.filterLabel}>
                                              {`4. Monto (${employeePayrollConfig?.moneda ?? 'MONEDA'})`}
                                            </div>
                                            <Tooltip
                                              title={
                                                !line.movimientoId ? 'Seleccione primero el movimiento' : undefined
          ...
          <div style={{ flexShrink: 0 }}>
            <div className={sharedStyles.companyModalFooter}>
              ...
              {!readOnly ? (
                ...
                >
                  {mode === 'create' ? 'Crear descuento' : 'Guardar cambios'}
                </Button>
              ) : null}
            </div>
          </div>
