...
export function isCoreTransactionLineComplete(line: CoreTransactionLine): boolean {
  ...
  const hasMonto = (line.montoInput ?? '').trim().length > 0 && line.monto != null && Number(line.monto) >= 0;
