...
import type { BonusType, PersonalActionAuditTrailItem } from '../../../../api/personalActions';
...
export interface BonusTransactionLine {
  key: string;
  payrollId?: number;
  payrollLabel?: string;
...
export interface BonusFormDraft {
  idEmpresa: number;
  idEmpleado: number;
  observacion?: string;
  lines: BonusTransactionLine[];
}

interface BonusTransactionModalProps {
  open: boolean;
  mode: 'create' | 'edit';
  title: string;
  ...
  initialDraft?: BonusFormDraft;
  onCancel: () => void;
  onSubmit: (payload: BonusFormDraft) => Promise<void> | void;
}
...
const BONUS_TYPE_OPTIONS: Array<{ value: BonusType; label: string }> = [
  { value: 'ordinaria_salarial', label: 'Ordinaria salarial' },
  { value: 'extraordinaria_habitual', label: 'Extraordinaria habitual' },
  { value: 'extraordinaria_ocasional', label: 'Extraordinaria ocasional' },
  { value: 'no_salarial_reembolso', label: 'No salarial / Reembolso' },
];

function buildEmptyLine(): BonusTransactionLine {
  return {
...
export function BonusTransactionModal({
  open,
  mode,
  title,
  companies,
  ...
  const handleAccept = async () => {
    ...
    if (lines.length === 0 || !lines.every(isLineComplete)) {
      message.error('Complete todas las líneas antes de crear/guardar la bonificación.');
      return;
    }
    ...
    modal.confirm({
      ...
      content:
        mode === 'create'
          ? '¿Está seguro de crear esta bonificación con las líneas capturadas?'
          : '¿Está seguro de guardar los cambios de esta bonificacion?',
          ...
          <div style={{ flexShrink: 0 }}>
            ...
            {mode !== 'edit' || activeTab === 'info' ? (
              ...
              <div className={sharedStyles.historicoSection}>
                <p className={sharedStyles.sectionTitle}>Historial de cambios de la bonificacion</p>
                ...
                <Table<PersonalActionAuditTrailItem>
                  ...
                  locale={{ emptyText: 'No hay registros de bitácora para esta bonificación.' }}
                />
              </div>
            )}
          </div>
          ...
          <div style={{ flexShrink: 0 }}>
            <div className={sharedStyles.companyModalFooter}>
              ...
              {!readOnly ? (
                ...
                >
                  {mode === 'create' ? 'Crear bonificacion' : 'Guardar cambios'}
                </Button>
              ) : null}
            </div>
          </div>
