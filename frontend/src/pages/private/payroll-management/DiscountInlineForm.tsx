/**
 * Formulario inline de Descuentos para la planilla.
 * Réplica la lógica y estilo del módulo Acción de Personal > Descuentos.
...
/** Salario por periodo según tipo de periodo (igual que DiscountTransactionModal). */
...
/** Línea del formulario de descuento. */
export interface DiscountInlineLine {
  key: string;
  movimientoId?: number;
...
export interface DiscountInlineFormProps {
  idEmpresa: number;
  idEmpleado: number;
  payrollId: number;
  ...
  onSubmit: (lines: UpsertDiscountLinePayload[]) => Promise<void>;
  onSuccess: () => void;
}

function buildEmptyLine(): DiscountInlineLine {
  return {
...
export function DiscountInlineForm({
  idEmpresa,
  idEmpleado,
  payrollId,
  employeeRow,
  ...
  const isLineComplete = useCallback((line: DiscountInlineLine): boolean => {
  ...
  const { lines, activeLineKeys, setActiveLineKeys, updateLine, addLine, removeLine } =
    useTransactionLines<DiscountInlineLine>({
      buildEmptyLine,
      isLineComplete,
      onIncompleteLine: () => {
  ...
  /** Filtra movimientos por empresa y tipo descuento (6); mantiene activos (esInactivo===1) o ya seleccionados. */
  const filteredMovements = useMemo(() => {
    let list = movements.filter((m) => Number(m.idEmpresa) === idEmpresa);
    list = list.filter((m) => m.idTipoAccionPersonal === 6);
   ...
   * Igual que DiscountTransactionModal.
   */
  const calculateLineAmount = useCallback(
    (line: DiscountInlineLine, movimientoId?: number, cantidadValue?: number) => {
  ...
  const buildPayloadLines = useCallback((): UpsertDiscountLinePayload[] => {
    const fechaEfecto =
  ...
  return (
    ...
    >
      <div style={{ fontWeight: 600, marginBottom: 12, color: '#3d4f5c' }}>Acción de Descuentos</div>
      <Collapse
