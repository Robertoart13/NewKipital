...
export function PayrollArticlesManagementPage() {
  ...
  const applyArticleToForm = useCallback(
    (row: PayrollArticleListItem) => {
      form.setFieldsValue({
  ...
  const loadPayrollAuditTrail = useCallback(
    async (id: number) => {
      if (!canViewAudit) {
        setAuditTrail([]);
  ...
  const onFormValuesChange = (changed: Partial<PayrollArticleFormValues>) => {
    ...
    if (
      'idEmpresa' in changed ||
      'idEmpresaCambio' in changed ||
      'idTipoArticuloNomina' in changed ||
      'idTipoArticuloNominaCambio' in changed
    ) {
      form.setFieldsValue({
        idCuentaGasto: undefined,
        idCuentaGastoCambio: undefined,
        idCuentaPasivo: undefined,
        idCuentaPasivoCambio: undefined,
      });
    }
  };
          ...
          filters={
            <PayrollArticlesFiltersPanel
              ...
              onPaneSelectionsChange={(key, selections) =>
                setPaneSelections((prev) => ({ ...prev, [key]: selections }))
              }
              paneOpen={paneOpen}
