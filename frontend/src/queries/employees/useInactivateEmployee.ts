...
export function useInactivateEmployee() {
  ...
  return useMutation({
    mutationFn: ({ id, motivo }: { id: number; motivo?: string }) => inactivateEmployee(id, motivo),
    onSuccess: (_, { id }) => {
    ...
    onError: (
      err: Error & {
        response?: {
          code?: string;
          planillas?: { id: number }[];
          acciones?: { id: number; tipoAccion?: string }[];
        };
      },
    ) => {
      const desc = err.response?.planillas?.length
      ...
      notification.error({
        message:
          err.response?.code === 'PLANILLAS_ACTIVAS'
            ? 'Planillas activas'
            : err.response?.code === 'ACCIONES_PENDIENTES'
